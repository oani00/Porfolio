...
select * from paciente;
select nompaciente,sexo from paciente;
...
rollback;  -- Desfaz (reverte) as alterações realizadas caso não tenhamsido efetivadas
commit;   -- efetiva as alterações no meio fisico do SGBD (disco)
...
alter table paciente modify endereco varchar(28) not null;
...
7.Sobre a tabela Paciente, alterar o campo endereço para ‘Rua Melo Alves,40’ e cidade para ‘Itu” para o paciente de código = 2. 
...
11.Alterar o campo desconto de todos os pacientes de sexo feminino e 
idade superior a 60 anos para ‘S’. 

dica: usar
select (sysdate - datanasc)/365.25 from paciente;
