...
-----------FATEC – ADS -   Laboratório de  Banco de Dados  - Tarefa 2 -----------------
----------------------------Lista02 – Visões em SQL------------------------------------
---------------------------------------------------------------------------------------
-- Entregar via Teams até o dia anterior a próxima aula (ver data no Teams)
...
CREATE OR REPLACE VIEW view-Consulta
AS
    SELECT codmedico, codpaciente, dataconsulta + 30 as novadata
    FROM Consulta;

-- 2. Crie uma visão que possua: nome do medico,código da consulta e dataconsulta.

CREATE OR REPLACE VIEW view-Consulta-2
AS
    SELECT m.nommedico, c.Codconsulta e c.dataconsulta
    FROM Consulta c
    INNER JOIN Medico m on c.Codmedico = m.Codmedico;

-- 3. Crie uma visão que exiba o código do medico e o valor total de suas consultas

CREATE OR REPLACE VIEW view-total-medicos
AS
    SELECT m.Codmedico, SUM(c.Valconsulta) as 'Total Consultas'
    FROM Consulta c
    INNER JOIN Medico m on c.Codmedico = m.Codmedico
    GROUP BY m.Codmedico;

-- 4. Crie uma visão que exiba o código do medico, o nome do medico  e o valor médio de suas consultas
CREATE OR REPLACE VIEW view-media-medicos
AS
    SELECT m.Codmedico, (SUM(c.Valconsulta) / COUNT(c.Valconsulta)) as 'Media Consultas'
    FROM Consulta c
    INNER JOIN Medico m on c.Codmedico = m.Codmedico
    GROUP BY m.Codmedico;
